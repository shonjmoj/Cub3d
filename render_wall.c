...
int	find_color_2(t_data *data, double wall, double offset_x, int from_top)
{
	...
	{
		offset_y = (from_top) * (double)(data->map_element.height[2] / wall);
		pixel_color = data->map_element.img_data[2][data->map_element.width[2] \
			* offset_y + (int)(offset_x * data->map_element.width[2])];
		return (pixel_color);
	}
	else if (data->rays[data->id].is_it_vertical
		&& !(data->alpha >= M_PI / 2 && data->alpha < 1.5 * M_PI))
